...
| 1.125   | `extensions.autoUpdate` simplified to `on` / `off`     | Old values (`true` / `false` / `onlyEnabledExtensions` / `delayed`) migrate automatically. A project that pins settings should pin the canonical `on` shape from this release forward; pre-1.125 heirs that have the old value still work via migration.                                                                                     |
| 1.125   | `extensions.autoUpdateDelay` configurable              | The 2-hour supply-chain delay introduced in 1.123 is now a configurable hour count. Alex ACT pins no value here, so heirs ride the platform default, but the setting exists if a heir or org wants tighter or looser update windows.                                                                                       |
...
| 1.128   | BYOK Claude via own Anthropic key                      | Heirs can run the Claude agent through their own Anthropic API credentials instead of consuming GitHub Copilot quota. Relevant wherever a project weighs the credit economy of model choice. No baseline setting; user-opt-in via Claude harness auth.                                                                          |
