...
function validateComponentPath(root, manifestRelativePath, field, value, findings) {
    if (typeof value !== 'string' || value.trim() === '') {
        findings.push({ severity: 'error', code: 'invalid-component-path', path: manifestRelativePath, message: `${field} must be a non-empty string path` });
        return;
    }
...
function validateManifestPaths(root, manifestRelativePath, manifest, findings) {
    ...
    for (const field of ['skills', 'prompts', 'agents', 'instructions']) {
        ...
        if (!Array.isArray(manifest.assets[field])) {
            findings.push({ severity: 'error', code: 'invalid-component-path', path: manifestRelativePath, message: `assets.${field} must be an array of paths or objects with a path field` });
            continue;
        }
        manifest.assets[field].forEach((value, index) => {
            const componentPath = value !== null && typeof value === 'object' && !Array.isArray(value)
                ? value.path : value;
            validateComponentPath(root, manifestRelativePath, `assets.${field}[${index}]`, componentPath, findings);
        });
    }
}
