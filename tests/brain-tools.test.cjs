...
const compilePrompt = bundledComponent('prompts/compile-brain.prompt.md', '.github/prompts/compile-brain.prompt.md', 'commands/compile-brain.md');
...
test('accepts ONE object-based manifest assets alongside string paths', () => {
    const root = targetFixture();
    write(path.join(root, 'manifest.json'), JSON.stringify({
        assets: {
            skills: [{ name: 'inspect', path: '.github/skills/inspect/SKILL.md' }],
            prompts: ['.github/prompts/inspect.prompt.md'],
            instructions: [],
            agents: [],
        },
    }));
    ...
    assert.equal(result.status, 0, result.stderr);
    const report = JSON.parse(result.stdout);
    assert.deepEqual(report.findings.filter((finding) => finding.path === 'manifest.json'), []);
});

test('rejects malformed and escaping object-based assets without losing their index', () => {
    const root = targetFixture();
    write(path.join(root, 'manifest.json'), JSON.stringify({
        assets: { skills: [
            { name: 'missing' }, { path: 42 }, { path: '' },
            { path: '../outside/SKILL.md' }, { path: 'absent/SKILL.md' },
        ] },
    }));
    const result = assess(root);
    assert.equal(result.status, 0, result.stderr);
    const findings = JSON.parse(result.stdout).findings;
    const codes = ['invalid-component-path', 'invalid-component-path', 'invalid-component-path',
        'component-path-escape', 'missing-component-path'];
    for (const [index, code] of codes.entries()) {
        assert.ok(findings.some((finding) => finding.code === code
            && finding.message.includes(`assets.skills[${index}]`)));
    }
});

test('runs both commands by absolute path from an unrelated adopter directory', () => {
    const cwd = temporaryDirectory('one-tools-adopter-');
    const target = targetFixture();
    const assessment = spawnSync(process.execPath, [assessor, '--root', target], { cwd, encoding: 'utf8' });
    assert.equal(assessment.status, 0, assessment.stderr);
    assert.equal(JSON.parse(assessment.stdout).immutability.preserved, true);
    const before = hashTree(cwd);
    const preview = spawnSync(process.execPath, [platformScaffolder, '--platform', 'all', '--target', cwd],
        { cwd, encoding: 'utf8' });
    assert.equal(preview.status, 0, preview.stderr);
    assert.equal(JSON.parse(preview.stdout).files.length, 8);
    assert.equal(hashTree(cwd), before);
});

test('preflights all scaffold destinations before rejecting an escaping directory junction', () => {
    const root = temporaryDirectory('one-scaffold-target-');
    const outside = temporaryDirectory('one-scaffold-outside-');
    junction(path.join(root, '.claude'), outside);
    const before = hashTree(root);
    const externalBefore = hashTree(outside);
    const result = scaffold(root, 'all', ['--apply', '--force']);
    assert.notEqual(result.status, 0);
    assert.match(result.stderr, /symbolic link|junction/i);
    assert.equal(hashTree(root), before);
    assert.equal(hashTree(outside), externalBefore);
});

test('rejects a dangling destination link before applying any scaffold files', () => {
    const root = temporaryDirectory('one-scaffold-target-');
    const outside = temporaryDirectory('one-scaffold-outside-');
    junction(path.join(root, '.claude'), path.join(outside, 'missing'));
    const result = scaffold(root, 'all', ['--apply']);
    assert.notEqual(result.status, 0);
    assert.match(result.stderr, /symbolic link|junction/i);
...
test('forced scaffold replacement does not change an external hard-linked file', () => {
    const root = temporaryDirectory('one-scaffold-target-');
    const outside = temporaryDirectory('one-scaffold-outside-');
    const original = path.join(outside, 'original.md');
    write(original, '# Preserve outside\n');
    fs.linkSync(original, path.join(root, 'AGENTS.md'));
    const result = scaffold(root, 'codex', ['--apply', '--force']);
    assert.equal(result.status, 0, result.stderr);
    assert.equal(fs.readFileSync(original, 'utf8'), '# Preserve outside\n');
    assert.match(fs.readFileSync(path.join(root, 'AGENTS.md'), 'utf8'), /^# Compile Brain/m);
});

test('invalid scaffold flags fail without creating files', () => {
    const root = temporaryDirectory('one-scaffold-invalid-');
    for (const args of [['--platform', 'unknown', '--target', root],
        ['--platform', 'copilot', '--target', root, '--force'],
        ['--platform', 'copilot', '--target']]) {
        const result = spawnSync(process.execPath, [platformScaffolder, ...args], { encoding: 'utf8' });
        assert.notEqual(result.status, 0);
        assert.match(result.stderr, /Unsupported platform|--force requires --apply|Missing value/);
        assert.deepEqual(fs.readdirSync(root), []);
    }
});

test('loading restored command modules does not run either CLI', () => {
    const cwd = temporaryDirectory('one-tools-import-');
    const result = spawnSync(process.execPath, ['-e',
        `require(${JSON.stringify(assessor)}); require(${JSON.stringify(platformScaffolder)});`],
    { cwd, encoding: 'utf8' });
    assert.equal(result.status, 0, result.stderr);
...
test('an incompatible late scaffold destination blocks the whole write set', () => {
    const root = temporaryDirectory('one-scaffold-types-');
    fs.mkdirSync(path.join(root, 'GEMINI.md'));
    const before = hashTree(root);
    const result = scaffold(root, 'all', ['--apply', '--force']);
    assert.notEqual(result.status, 0);
...
test('default apply preserves a file created after the initial overwrite check', () => {
    const root = temporaryDirectory('one-scaffold-race-');
    const injection = path.join(temporaryDirectory('one-scaffold-race-hook-'), 'inject.cjs');
    ...
    write(injection, [
        ...
        '  const result = original.call(this, directory, options);',
        `  if (!injected && directory === ${JSON.stringify(root)}) {`,
        '    injected = true;',
        `    fs.writeFileSync(${JSON.stringify(destination)}, '# Concurrent owner\\n');`,
        '  }',
        '  return result;',
        '};',
    ].join('\n'));
    const result = spawnSync(process.execPath, ['--require', injection, platformScaffolder,
        '--platform', 'codex', '--target', root, '--apply'], { encoding: 'utf8' });
    assert.notEqual(result.status, 0);
    assert.match(result.stderr, /EEXIST|already exists/);
    assert.equal(fs.readFileSync(destination, 'utf8'), '# Concurrent owner\n');
