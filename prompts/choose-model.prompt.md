...
5. If the selected model needs a credential, emit a `Secret setup` section with
	the exact host environment variable or approved secret-storage route; never
	request or print its value.
...
if it invokes provider execution before handing off, or if its provider-neutral
planning evidence does not justify retaining the optional plugin.
